...
/* Define a structure to store device information */
struct net_device {
    ...
    /* Hardware address of the device, etc. */
    union {
        uint8_t peer[NET_DEVICE_ADDR_LEN];
        uint8_t broadcast[NET_DEVICE_ADDR_LEN];
    };
    struct net_device_ops *ops; /* Pointer to struct net_device_ops where the function implemented in the device driver is set */
    void *priv; /* Pointer to private data used by device driver */
};

/* Stores pointers to functions implemented in device drivers */
